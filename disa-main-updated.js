class DisaMain extends Polymer.Element {
  ...
  optionsReceived(e) {
    ...
    if (!response) {
      ...
      let res_typekind = response[3]['typeKindOfEnslavement'];
      for (let i = 0; i < res_typekind.length; i++) {
        if (res_typekind[i] === 'Man servant' || res_typekind[i] === 'Woman servant') {
          res_typekind[i] = 'Servant';
        } else if (res_typekind[i] === 'Man slave' || res_typekind[i] === 'Woman slave') {
          res_typekind[i] = 'Slave';
        } 
        else if (res_typekind[i].startsWith('Indenture --')) {
          ;
        } else if (res_typekind[i].startsWith('Indenture')) {
          res_typekind[i] = 'Indenture';
        }
      }
      response[3]['typeKindOfEnslavement'] = response[3]['typeKindOfEnslavement'].filter(onlyUnique);

      let res_race = response[7]['race'];
      for (let i = 0; i < res_race.length; i++) {
        if (res_race[i] === 'Indian Mulatto') {
          ;
        } else if (res_race[i].endsWith('Mulatto')) {
          res_race[i] = 'Mulatto';
        } else if (res_race[i].endsWith('Indian')) {
          res_race[i] = 'Indian';
        } else if (res_race[i] == 'Criollo') {
          res_race[i] = 'Other';
        }
      }
      response[7]['race'] = response[7]['race'].filter(onlyUnique);
